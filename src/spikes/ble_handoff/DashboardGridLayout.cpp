...
void computeGridLayout(const WidgetGridPackage& package, const int canvasWidth, const int canvasHeight,
                       std::array<WidgetRect, MAX_WIDGETS>& rectsOut, const int originX, const int originY) {
  ...
  for (uint8_t index = 0; index < package.widgetCount; ++index) {
    ...
    rectsOut[index] = WidgetRect{originX + widget.column * colWidth, originY + widget.row * rowUnitHeight,

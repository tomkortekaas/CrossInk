...
constexpr uint8_t SLEEP_SCREEN_STORAGE_ORDER[] = {
    ...
    static_cast<uint8_t>(CrossPointSettings::PERSONAL_DASHBOARD_SLEEP),
};
...
bool CrossPointSettings::verifySleepScreenMigrationContract() {
  ...
  constexpr uint8_t personalDashboardStorageValue = 12;
  ...
  return sleepScreenStorageToMode(minimalSleepStorageValue) == MINIMAL_SLEEP &&
         ...
         sleepScreenStorageToMode(personalDashboardStorageValue) == PERSONAL_DASHBOARD_SLEEP &&
         sleepScreenModeToStorage(PERSONAL_DASHBOARD_SLEEP) == personalDashboardStorageValue &&
